				...
				function (cb) {
					if (!plans || !expiry || expiry < now) {
						db.plans(function (err,p) {
							...
							_.each(p.plans||p,function (plan) {
								...
								limits = _.keys(plan.limits);
								resources.push.apply(resources,limits);
							});
					...
					if (resource) {
						db.user(user,function (err,u) {
							var limit, usage, plan;
							if (err) {
								res.send(400,err);
							} else {
								if (u && u.plan) {
									plan = u.plan.name || u.plan;
								}
								if (plans.plan[plan] && plans.plan[plan].limits && plans.plan[plan].limits[resource] !== undefined) {
									limit = plans.plan[plan].limits[resource];
								}
								...
								if (limit === undefined || limit === null) {
									...
									res.send(403,{reason:"subscription",plan:plan,limit:resource,maximum:limit});
								} else {
									cb();
								}
							}
